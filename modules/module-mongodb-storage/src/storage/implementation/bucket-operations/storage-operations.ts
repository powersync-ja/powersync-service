...
import { getBucketDataBatchShared } from './read-operations.js';

export interface BucketDataBatchContext {
  db: {
    bucketData: (groupId: number, definitionId: string) => mongo.Collection<any>;
  };
  group_id: number;
  mapping: {
    bucketSourceId: (source: storage.BucketDataRequest['source']) => string;
  };
}

export async function* getBucketDataBatchSharedWrapper(
  ctx: BucketDataBatchContext,
  ...
  for (let groupIndex = 0; groupIndex < definitionGroups.length && remainingLimit > 0; groupIndex++) {
    ...
    const {
      ...
      documentOpCounts,
      documentSizes
    } = await getBucketDataBatchShared({
      collection: ctx.db.bucketData(
        ctx.group_id,
        definitionId
...
export interface ParameterSetsContext {
  db: {
    client: mongo.MongoClient;
    parameterIndex: (groupId: number, indexId: string) => mongo.Collection<any>;
  };
  group_id: number;
}

export async function getParameterSetsShared(
  ctx: ParameterSetsContext,
  checkpoint: MongoSyncBucketStorageCheckpoint,
  lookups: ScopedParameterLookup[],
  serializeParameterLookup: (lookup: ScopedParameterLookup) => bson.Binary
  ...
  return ctx.db.client.withSession({ snapshot: true }, async (session) => {
    ...
    const buildLookupPipeline = (
      ...
      const collection = ctx.db.parameterIndex(ctx.group_id, indexId);
...
export interface DataBucketChangesContext {
  db: {
...
export async function getDataBucketChangesShared(
  ctx: DataBucketChangesContext,
  options: GetCheckpointChangesOptions
): Promise<Pick<CheckpointChanges, 'updatedDataBuckets' | 'invalidateDataBuckets'>> {
  const limit = 1000;
  const bucketStateUpdates = await ctx.db
    .bucketState(ctx.group_id)
...
export interface ParameterBucketChangesContext {
  db: {
    parameterIndex: (groupId: number, indexId: string) => mongo.Collection<any>;
  };
  group_id: number;
  mapping: {
    allParameterIndexIds: () => string[];
  };
}

export async function getParameterBucketChangesShared(
  ctx: ParameterBucketChangesContext,
  options: GetCheckpointChangesOptions,
  deserializeParameterLookup: (lookup: bson.Binary, indexId: string) => any
): Promise<Pick<CheckpointChanges, 'updatedParameterLookups' | 'invalidateParameterBuckets'>> {
  const limit = 1000;
  const indexIds = ctx.mapping.allParameterIndexIds();
  const collections = indexIds.map((indexId) => ({
    indexId,
    collection: ctx.db.parameterIndex(ctx.group_id, indexId)
  }));
