...
bucket_definitions:
  global:
    ...
    // Create a large table so the snapshot takes multiple chunks
    await pool.query(
      `CREATE TABLE test_data(id uuid primary key default uuid_generate_v4(), description text)`
    );
    await pool.query(`INSERT INTO test_data(description) SELECT 'row ' || g FROM generate_series(1, 50000) g`);
