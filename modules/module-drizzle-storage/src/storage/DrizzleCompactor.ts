...
export interface DrizzleCompactOptions extends storage.CompactOptions {
  logger: Logger;
}
...
export class DrizzleCompactor {
  ...
  private async executeAll<T>(sql: string, params: unknown[]): Promise<T[]> {
    return this.dialect.db.$client.prepare(sql).all(...params) as T[];
  }

  private async executeRun(sql: string, params: unknown[]): Promise<void> {
    this.dialect.db.$client.prepare(sql).run(...params);
  }
}
