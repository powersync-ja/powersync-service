...
import { compileToSyncPlan, compileToSyncPlanWithoutErrors } from './utils.js';
...
describe('common table expressions', () => {
    ...
    const withDirectReference = compileToSyncPlanWithoutErrors([
      {
        name: 'stream',
        ctes: {
          org_of_user: `SELECT id FROM organisations
                WHERE id IN (SELECT org_id FROM org_memberships WHERE user_id = auth.user_id())`
        },
        queries: ['SELECT * FROM projects WHERE org_id IN org_of_user']
      }
    ]);
  ...
  describe('errors', () => {
    test('shorthand syntax for CTE with multiple columns', () => {
      const [errors, _] = compileToSyncPlan([
        {
          name: 'stream',
          ctes: {
            org_of_user: `SELECT id, name FROM organisations
