...
export function stringify(
  ...
  const replacedValue = typeof replacer === 'function' ? replacer.call({ '': value }, '', value as any) : value;
   ...
   * Stringify an array
   */
  function stringifyArray(array: Array<any>, indent: string | undefined): string {
    if (array.length === 0) {
      return '[]';
    }
  ...
  function stringifyObject(object: GenericObject<unknown>, indent: string | undefined): string {
    ...
    keys.forEach((key) => {
      const value = typeof replacer === 'function' ? replacer.call(object, key, object[key] as any) : object[key];
