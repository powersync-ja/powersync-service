// Adapted from https://github.com/josdejong/lossless-json/blob/main/tools/benchmark/run.mjs

import { JSONBig } from './dist/index.js';
...
const losslessJSON = JSONBig.parse(text);

assert.deepStrictEqual(JSON.parse(JSONBig.stringify(losslessJSON)), json);

const suite = new Benchmark.Suite('parse and stringify benchmark');
suite
  .add('   JSON.parse    ', () => JSON.parse(text))
  .add('JSONBig.parse    ', () => JSONBig.parse(text))
  .add('   JSON.stringify', () => JSON.stringify(json))
  .add('JSONBig.stringify', () => JSONBig.stringify(losslessJSON))
