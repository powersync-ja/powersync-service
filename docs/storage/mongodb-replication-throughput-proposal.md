...
MongoDB full rows now use one preparation worker per writer for BSON conversion, data/parameter evaluation, JSON encoding, PUT checksums, replica identity extraction, subkeys and REMOVE checksums. Messages copy raw BSON and encoded JSON strings; identities return as small BSON buffers, preserving their types without cloning BSON objects. The main thread reconstructs identities for storage queries, reconciles memberships, allocates operation IDs and publishes. The worker prepares block N+1 while the main thread applies block N, with at most one preparing/prepared block ahead of application and one accumulating source block. Membership reads, reconciliation, operation allocation and publication admission remain ordered; page and checkpoint boundaries join application before sealing. Event tables and paths requiring current-row reconstruction remain inline. Worker errors abort dependent publication, and disposal cancels the worker and uploads.

Concurrent snapshots, worker pools, multiple simultaneous preparation requests and membership-write coalescing remain future optimizations. The architecture below describes that broader direction. Tests cover combined preparation blocks, overlapping pages/uploads, backpressure, ordered resume positions and visibility, membership dependencies, publication retries and orphan cleanup. The 30 MB/s target remains unvalidated.
