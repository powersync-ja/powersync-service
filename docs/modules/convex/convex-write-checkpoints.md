...
Terminology: This feature is now called checkpoint requests. Older code, this
file name, storage APIs, and the sync protocol field `write_checkpoint` still use
"write checkpoint" for backwards compatibility.

## Background

Checkpoint requests let a client wait until its own uploaded write has been
...
same effective ordering: the client must not be left with a stored checkpoint
request id and no later observable checkpoint update.
...
checkpoint LSN is at or past the checkpoint request head, the sync stream can
acknowledge the checkpoint request to the client.

## Case 1: no checkpoint collection

If there is no `powersync_checkpoints` collection, PowerSync can still read the
current Convex head and create a checkpoint request in storage. The problem is
that nothing guarantees the Convex delta stream will advance beyond that head.

### Case 1.1: no replication lag

If there is no replication lag, the latest Convex head may already include the
client's source write by the time `/sync/checkpoint-request` runs. The legacy
`/write-checkpoint2.json` route has the same source-position requirement.
PowerSync can therefore associate the checkpoint request id with a correct
source head.

However, correctness of the association is not enough. The connected sync client
only learns about acknowledged checkpoint requests when PowerSync sends a later
sync checkpoint whose source LSN is at or beyond the stored checkpoint request
head.

If the replicator has already processed past that head before the checkpoint
request mapping is created, there may be no future replication event to cause a
commit or keepalive. On an idle Convex app, repeated `document_deltas` calls can
...
### Case 1.2: manual checkpoint request creation

Manually calling `/sync/checkpoint-request`, or the legacy
`/write-checkpoint2.json` route, has the same shape. The checkpoint request is
associated with the current Convex head, but if no later source change occurs,
the stream does not advance. The connected client will not see the checkpoint
request acknowledgement until an unrelated external change moves the Convex
cursor and replication commits or keeps alive at that newer cursor.
...
### Case 2.1: marker advances past the checkpoint request head

The checkpoint request is associated with the pre-marker head. The marker
mutation is committed after that head, so its delta cursor is greater than or
equal to the point needed to acknowledge the checkpoint request.
...
checkpoint LSN past the checkpoint request head, allowing the checkpoint request
id to be sent to the client.

This works regardless of whether the normal user-data write was already
replicated before `/sync/checkpoint-request` ran, as long as the marker-driven
checkpoint update is observed after the checkpoint request mapping exists. That
is the event that makes the stored checkpoint request visible to sync.
...
The common requirement is not "checkpoint request data must be replicated as user
data". The requirement is that, after creating the checkpoint request mapping,
the source stream must produce an ordered event beyond the stored position.
...
3. The client then submits a checkpoint request.
...
will not observe an event at or beyond the stored checkpoint request LSN, even
though the source database head has advanced. The checkpoint request can then be
...
other writes after the checkpoint request mapping is created. The marker is
still required because an idle Convex deployment may otherwise keep returning the
same delta cursor, but the reason is different from a filtered Postgres slot
that cannot observe some source LSNs at all.
...
### Test 1: direct checkpoint request on an idle source

When `/sync/checkpoint-request` was called directly without any other client
mutation:

- without writing to `powersync_checkpoints`, the checkpoint request did not show
  up immediately in the client logs or PowerSync service sync logs,
- with the `powersync_checkpoints` marker write enabled, the checkpoint request
  did show up immediately.

This confirms the idle-source failure mode. A checkpoint request can be stored
in PowerSync bucket storage, but without a later Convex delta there may be no
sync checkpoint diff that exposes it to the connected client.

### Test 2: replication lag

When replication lag was introduced, the checkpoint request only appeared after
replication caught up.

This is the desired behavior. The client should not validate a checkpoint
request until PowerSync has replicated to a source position at or beyond the head
stored for that checkpoint request.
...
### Test 4: delayed checkpoint request association

Another test disabled the `powersync_checkpoints` marker write and added a
2-second delay in the Convex API handler before creating the checkpoint request.

In that setup, the client did not receive a checkpoint associated with the
checkpoint request. The likely sequence is:

1. the client mutation committed in Convex,
2. the replicator saw the corresponding Convex cursor,
3. PowerSync sent the sync checkpoint to the client before the checkpoint
   request mapping existed,
4. the API handler created the checkpoint request after the delay,
...
replicated, but the connected client can still miss the checkpoint request
acknowledgement if the checkpoint request mapping is created after the relevant
sync checkpoint has already been sent.

## Conclusion

With the current Convex replication API and checkpoint request design, the write
to `powersync_checkpoints` is required.
...
writes, and checkpoint requests are only acknowledged after bucket storage
commits or keeps alive at a cursor at or beyond the stored head. Without the
marker write, a checkpoint request can be associated with a correct Convex head
but never become visible to the connected client on an idle source, especially
when replication has already processed past that head before the checkpoint
request mapping was stored.
...
created checkpoint request against an already-committed source head. Until then,
`CONVEX_CHECKPOINT_TABLE` is the mechanism that makes checkpoint requests
reliably observable. The implementation should also preserve the
ordering invariant above; otherwise the marker can be replicated before the
checkpoint request mapping exists, recreating the same idle-source stall in a
narrower race.
