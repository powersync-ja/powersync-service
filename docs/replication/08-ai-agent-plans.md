...
The plans describe phase goals, responsibilities, and acceptance checks. Use the source interfaces and their JSDoc for exact method-level behavior.
...
- [Source connector overview](./03-source-connector-overview.md)
- [Replication lifecycle](./02-replication-lifecycle.md)
- [Snapshots and streaming](./05-snapshots-and-streaming.md)
- [Storage writer overview](./04-storage-writer-overview.md)
...
2. Read that module's registration, route adapter, replicator, replication job, and stream helper.
...
4. Implement the module subclass with route API creation, replicator creation, connection testing, and any source-specific setup after registration.
...
6. Implement a replicator subclass that can create jobs, clean up stopped streams idempotently, and run connection checks.
7. Implement a replication job subclass whose replication entrypoint creates an `ExampleDBStream` helper and whose keepalive and lag behavior is explicit about what is supported.
...
3. Convert source metadata into the common schema format where meaningful. For schemaless sources, return the best available collection/table list and type metadata, or document which strict schema fields are unavailable.
4. Make sync config table patterns resolve into concrete source tables or collections with useful warnings.
5. Provide parsing defaults for default database, default schema, identifier behavior, schemaless behavior, or other source-specific sync config options.
...
10. Add an integration test using the module test context that creates representative source schema or collections and verifies schema extraction, table diagnostics, and sync config validation expose the available metadata and warnings.
...
Goal: copy existing source data through the storage writer while preserving the initial consistency boundary.

Steps:

1. Open a storage writer with the correct source zero position, current-data behavior, snapshot-resume behavior, hooks, and tracer options.
...
4. Resolve each discovered entity through storage.
5. Remove outdated source table mappings returned by resolution.
6. Copy source rows in chunks, pages, or cursor batches.
7. Normalize each row into an insert-like source change for the resolved `SourceTable` records.
8. Persist progress after chunks so interrupted snapshots can resume.
9. Mark each table snapshot complete with the source position that checkpoints must not precede.
10. Mark the overall snapshot complete once every required table is complete.
11. Advance the source position at a source-safe boundary so storage can unblock checkpoints after streaming reaches the boundary.
...
- Re-running a chunk is safe when resumable snapshot mode skips already-copied rows.
...
2. Resume from the source-managed stream state or from the storage-managed restart cursor, according to the ownership model defined in Plan 1.
...
5. Resolve source table metadata when a source entity is first seen or when metadata may have changed.
6. Remove or truncate source table mappings when they are superseded, dropped, renamed, or require a fresh snapshot.
7. Normalize inserts, updates, and deletes into storage writer row changes.
8. Flush partial work inside large source transactions when needed, but only advance a checkpoint after a transaction-safe boundary.
9. Advance position without user data for heartbeats, checkpoint markers, or idle-source progress.
10. Persist PowerSync-managed resume tokens only for restart safety; this must not by itself make that source position visible as a checkpoint.
11. Report replication lag from source commit timestamps or the closest reliable source metadata.
12. Detect missing or expired history and request a fresh replication stream rather than skipping changes.
...
- Checkpoint advancement never splits a source transaction across visible PowerSync checkpoints.
- Idle source marker events can advance checkpoints without writing user data.
- Restart from the source-managed state or persisted restart cursor does not duplicate visible effects incorrectly or skip changes.
...
4. Test large source transactions and confirm partial flushes do not expose transaction results before a checkpoint-safe boundary.
...
- Storage-facing tests run against real storage modules, so checkpoint, parameter lookup, current-data, and bucket operation behavior are covered by the actual storage workflow.
...
The most useful prompts name a single phase, a reference module, and the source-specific decisions that are already known. That keeps the agent focused on one responsibility boundary at a time.
