...
saved configs. Modules register additional parsers during initialization, before storage starts. Registration validates
an isolated schema composition before publishing it; an invalid registration leaves the current parser unchanged.
...
empty: both `additionalProperties: false` and `maxProperties: 0` explicitly reject options that require an external
module. An empty connection `{}` is normalized away. A nonempty connection requires `type` and cannot contain unknown
...
core. Modules inspect `type` before using their options. Table declarations configure SQL-selected sources; they do not
select additional replication sources. Unqualified names remain relative to the source connection's default schema.
...
- `extendJsonSchema({ schema })` can specialize the full resolved JSON schema. Preserve validation of unrelated fields
  and connection types. Tooling reads a copy of the same schema through `syncConfigParser.jsonSchema`.
- `parse({ config, context })` receives decoded YAML. The context provides the candidate parsed config, SQL-selected
  source tables, default schema, source-location lookup, and diagnostic reporting. Store additional options on the appropriate
  config fields, such as `connectionConfig`. A fatal diagnostic rejects the candidate, including in diagnostic-only parsing mode.
- `validatePersisted({ config, context })` validates saved config fields without reparsing SQL or relying on YAML
  locations. Modules with semantic restrictions beyond their JSON schema must repeat those checks here.
...
same YAML locations, including escaped JSON-pointer segments. `patternErrorMessage` is accepted as an editor annotation;
...
Nonempty connection options use sync-plan format 4 so an older service rejects options it cannot interpret. Unfiltered
plans retain formats 1 and 2. Format 3 belonged to the exploratory extension payload and is not accepted here.

Loading a saved plan validates connection options against the currently composed schema before running module semantic
validators and hydrating it. If the required module is absent, loading fails. A failed saved plan is never replaced by
silently reparsing the original YAML.

Connection maps must match before configs share incremental processing or are hydrated together. Equality normalizes
