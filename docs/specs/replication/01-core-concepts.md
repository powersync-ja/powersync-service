...
If PowerSync only needed one-way ordered replication, the internal operation id sequence would be enough. Source positions are needed for additional source-facing guarantees:
...
- Most non-Postgres sources need a persisted resume position after restarts. Postgres replication slots track most resume state on the source side, so `setResumeLsn()` is less central there.
...
For a fuller walkthrough, see [resolve-tables-flow.md](../../resolve-tables-flow.md).
...
The client-facing behavior is defined in [sync-protocol.md](../../sync-protocol.md), while formal bucket invariants are described in [bucket-properties.md](../../bucket-properties.md).
...
This lets the sync API ask, at a given checkpoint, which dynamic buckets a client should receive. See [parameters-lookups.md](../../parameters-lookups.md) for the storage model and compaction concerns.
